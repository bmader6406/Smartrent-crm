...
class NonYardiMasterLoader
  ...
  def self.perform(time, import_id);
    ...
    begin
      ...
      meta["incremental_upload"] = 1 # load 10k rows each time for now (no need to fetch the new rows between two feed)
      
      # for logging only
      log = import.logs.create(:file_path => file_name)
      
      meta["import_log_id"] = log.id
      
      Resque.enqueue(ResidentImporter, tmp_file, "non_yardi_master", import.field_map, meta)
      ...
      Notifier.system_message("[CRM] NonYardiMasterLoader FAILURE", "ERROR DETAILS: #{error_details}",
