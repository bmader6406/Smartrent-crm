...
(function($){
	...
	var Redactor = function(element, options)
	{
      ...
      style: '', //TIN added
			...
			allowedTags: ["head", "link", "meta", "style", "script", //TIN added this line
			    "form", "input", "button", "select", "option", "datalist", "output", "textarea", "fieldset", "legend",
	...
	Redactor.prototype = {
		...
		build: function(mobile, whendone)
		{
			if (mobile !== true)
			{
				...
				{
				  //TIN edited
				  var box = '<div class="hl box">' +
				              this.opts.beginTags + '<div id="page" contenteditable="true" dir="' + this.opts.direction + '">' + '</div>' + this.opts.endTags;
				            '</div>';
				  
				  var style = '<style type="text/css" media="screen"> ' +  
				                '#page { margin: 10px; outline: 0 none; } ' + this.opts.style + 
				              '</style>';
				  
				  this.$content.contents().find('head').append(style);
				  
				  this.$editor = this.$content.contents().find("body").html(box).find('#page');
      ...
      // $(dropdown).append(elnone);
			...
			//TIN added custom color, must have colorpicker plugin
			// var _self = this,
			//         pickerTimeout = null,
			//         elcustom = $('<a href="javascript:void(null);" class="redactor_color_none"></a>').html('Custom');
			//       elcustom.ColorPicker({
			//         onShow: function (picker) {
			//           var offset = elcustom.offset();
			//           
			//           $(picker).addClass('in-toolbar').css({top: offset.top - 150, left: offset.left - 130});
			//           _self.hideAllDropDown();
			//         },
			...
			//             _self.execCommand(mode, "#" + hex);
			//             _self.saveSelection();
			//           }, 100);
			//         }
			//       });
			//       
			//       $(dropdown).append(elcustom);
		...
		hideDropDown: function(e, dropdown, key)
		{
			if (!$(e.target).hasClass('dropact'))
			{
				...
				//TIN hide picker
				if(!$(e.target).parents('.in-toolbar')[0] && !$(e.target).hasClass('in-toolbar')){
				  $('.colorpicker.in-toolbar:visible').hide();
				}
			}
		},
		...
		showLink: function()
		{
			...
			var callback = $.proxy(function()
			{
				...
				if (target === '_blank')
				{
					$('#redactor_link_blank').prop('checked', true); //TIN edited (attr -> prop)
				}
		...
		insertLink: function()
		{
			...
			if (tab_selected === '1') // url
			{
				...
				if ($('#redactor_link_blank').prop('checked')) //TIN edited (attr -> prop)
				{
